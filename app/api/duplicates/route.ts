  ...
  try {
    ...
    const byFilename = await sql`
      SELECT
        ...
        count(*)::int as cnt
      FROM entries
      WHERE original_filename IS NOT NULL AND file_hash IS NULL
      GROUP BY original_filename
      HAVING count(*) > 1
    `;

    const bySeed = await sql`
      SELECT
        min(trim(prompt)) as key_prompt,
        null as filename,
        array_agg(id ORDER BY created_at ASC) as ids,
      ...
      GROUP BY trim(model), (comfy_settings::jsonb->>'seed')
      HAVING count(*) > 1
    `;

    const rows = [...byHash, ...byFilename, ...bySeed]
